...
module.exports = function (opts = {}, app) {
  ...
  o.middleware = middleware(opts.wechat)
    ...
    .done(function (message, req, res, next) {
      ...
      o.pay
        ...
        .then(function (doc) {
          if (doc && doc._id) return o.pay.post('/pays/' + doc._id, data)
          return null
        })
        .then(function (doc) {
          if (doc) {
  ...
  o.refundMiddleware = middleware(opts.wechat)
    ...
    .done(function (message, req, res, next) {
      ...
      let refundId = message.out_refund_no
      res.reply('success')
      /**
       * 有错误返回错误，不然微信会在一段时间里以一定频次请求你
       * res.reply(new Error('...'))
      ...
      o.pay
        ...
        .then(function (doc) {
          if (doc) {
            ...
            logger.warn('无效退款单, 编码: %s', refundId)
            return null
          }
        })
        .then(function (doc) {
          logger.debug(doc)
          if (doc && doc._id){
            o.pay.post('/pays/' + doc.pay._id, {status: 3}).then(function(){
        ...
        .then(function (doc) {
          if (doc) {
            logger.info('退款单编码 %j 修改为已退款', refundId)
          }
        })
    })
